...
const ArrayConfig = ({ arrays, setArrays, activeArrayIndex, setActiveArrayIndex }) => {
    const addArray = () => {
        setArrays(prev => [...prev, {
    ...
    const removeArray = (id) => {
        ...
        setArrays(newArrays);
        if (activeArrayIndex >= newArrays.length) setActiveArrayIndex(newArrays.length - 1);
    };

    const updateArray = (field, value) => {
        setArrays(prev => prev.map((a, i) => 
    ...
    const loadScenario = (name) => {
        ...
        if (name === '5G') {
            setArrays([{ ...base, count: 32, geo: 'linear', curve: 0, steering: 30, x: 0, y: 0 }]);
        } else if (name === 'Ultrasound') {
            setArrays([{ ...base, count: 64, geo: 'curved', curve: 8, steering: 0, x: 0, y: 0 }]);
        } else if (name === 'Tumor') {
            setArrays([{ ...base, count: 16, geo: 'curved', curve: 15, steering: 0, x: 0, y: 0 }]);
        }
            ...
            <div style={{ padding: '15px', overflowY: 'auto', flex: 1 }}>
                ...
                <div style={{ marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                        <button style={btnStyle(false)} onClick={() => loadScenario('5G')}>5G</button>
                        <button style={btnStyle(false)} onClick={() => loadScenario('Ultrasound')}>SONAR</button>
                        <button style={btnStyle(false)} onClick={() => loadScenario('Tumor')}>HEAL</button>
                    </div>
                </div>

                {/* Array List */}
                <div style={{ marginBottom: '15px' }}>
                    <div style={{ fontSize: '0.7rem', color: '#aaa', marginBottom: '8px', fontFamily: 'Cinzel' }}>ACTIVE ARRAYS</div>
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                        {arrays.map((a, i) => (
                            <button
                                ...
                                style={{
                                    padding: '4px 8px', fontSize: '0.6rem', border: '1px solid #333',
                        ...
                        <button
                            onClick={addArray}
                            style={{
                                padding: '4px 8px', fontSize: '0.6rem', border: '1px dashed #555',
                                background: '#111', color: '#666', fontFamily: 'Cinzel'
                            }}
                        >
                            +
                        </button>
                        {arrays.length > 1 && (
                            <button
                                ...
                                style={{
                                    padding: '4px 8px', fontSize: '0.6rem', border: '1px solid #d32f2f',
                                    background: '#222', color: '#d32f2f', fontFamily: 'Cinzel'
                                }}
                            >
                                ✕
                            </button>
                ...
                {arrays.length > 0 && (
                    <div style={{ background: 'rgba(0,0,0,0.3)', padding: '10px', border: '1px solid #333', borderRadius: '4px' }}>
                        <div style={{ marginBottom: '10px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: '#aaa', fontFamily: 'Lato' }}>
                                <span>Mana Sources</span><span style={{color: '#c5a059'}}>{current.count}</span>
                            </div>
                            <input
                        ...
                        <div style={{ marginBottom: '10px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: '#aaa', fontFamily: 'Lato' }}>
                                <span>Steering</span><span style={{color: '#d32f2f'}}>{current.steering}°</span>
                            </div>
                            <input
                                type="range" min="-90" max="90"
                                value={current.steering}
                                onChange={e => updateArray('steering', parseInt(e.target.value))}
                            />
                        </div>

                        <div style={{ marginBottom: '10px' }}>
                            <div style={{ fontSize: '0.65rem', color: '#aaa', marginBottom: '4px' }}>Formation</div>
                            <select
                                value={current.geo}
                                onChange={e => updateArray('geo', e.target.value)}
                                style={{ width: '100%', background: '#0a0a0c', color: '#e0e0e0', border: '1px solid #444', padding: '6px', fontFamily: 'Cinzel', fontSize: '0.7rem' }}
                            >
                        ...
                        {current.geo === 'curved' && (
                            <div style={{ marginBottom: '10px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: '#aaa', fontFamily: 'Lato' }}>
                                    <span>Curvature</span><span>{current.curve}</span>
                                </div>
                                <input
                                    type="range" min="1" max="20"
                                    value={current.curve}
                                    onChange={e => updateArray('curve', parseInt(e.target.value))}
                                />
                            </div>
                        )}
                        ...
                        <div style={{ marginTop: '10px', borderTop: '1px solid #333', paddingTop: '10px' }}>
                            <div style={{ fontSize: '0.65rem', color: '#aaa', marginBottom: '4px' }}>Position (m)</div>
                            ...
                            <div style={{ display: 'flex', gap: '8px', fontSize: '0.65rem', marginTop: '5px' }}>
